...
namespace Saving.CriteriaIReport.u_cri_coopid_loancont_payment
{
    public partial class DsMain : DataSourceFormView
    {
        ...
        public void DdLoancont(string memnno)
        {
            String sql = @"select loancontract_no,1 as sorter from lncontmaster where member_no = {0} and principal_balance > 0 
                             union
select '',0 from dual order by sorter,loancontract_no";
            sql = WebUtil.SQLFormat(sql,memnno);
            DataTable dt = WebUtil.Query(sql);
