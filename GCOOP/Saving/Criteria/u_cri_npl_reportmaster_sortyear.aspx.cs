...
{
    public partial class u_cri_npl_reportmaster_sortyear : PageWebSheet, WebSheet
    {
        ...
        public void WebSheetLoadBegin()
        {
            if (IsPostBack)
            {
                ...
                this.RestoreContextDw(dw_lawtype);
            }
            else
            {
                dw_criteria.InsertRow(0);

                dw_lawtype.InsertRow(0);
                dw_lawtype.SetItemDecimal(1, "year_th", state.SsWorkDate.Year + 543);
                for (int i = 0; i <= 7; i++)
                {
                    dw_lawtype.SetItemDecimal(1, "lawtype_" + i, 0);
                }
                for (int i = 1; i <= 4; i++)
                {
                    dw_lawtype.SetItemDecimal(1, "lawtype_" + i, 1);
                }
        ...
        private void RunProcess()
        {
            ...
            for (int i = 0; i <= 7; i++)
            {
                if (dw_lawtype.GetItemDecimal(1, "lawtype_" + i) == 1)
                {
                    array += "," + i;
                }
            }
