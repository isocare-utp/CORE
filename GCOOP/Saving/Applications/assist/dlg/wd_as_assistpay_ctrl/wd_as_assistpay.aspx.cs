    ...
    public partial class wd_as_assistpay : PageWebDialog, WebDialog
    {
        ...
        public void CheckJsPostBack(string eventArg)
        {
            ...
            else if (eventArg == PostGetFee)
            {
                ...
                dsPayto.FindDropDownList(li_row, "tofrom_accid").Enabled = true;
        ...
        private void of_setpaytodesc()
        {
            ...
            ls_deptaccno = dsPayto.DATA[li_row].DEPTACCOUNT_NO;
            ls_sendsystem = dsMain.DATA[0].send_system;
            ...
            else if (ls_moneytype == "TRN")
            {
                ...
                else
                {
                    ...
                    dsPayto.DATA[li_row].DEPTACCOUNT_NO = ls_deptaccno;
                }
            }
            //dsPayto.DATA[li_row].DESCRIPTION = ls_desc;
            if (dsPayto.RowCount > 0) {
                decimal rc = 0;
                for (int i = 0; i < dsPayto.RowCount; i++) {
                    if (li_row != i) {
                        string description = dsPayto.DATA[i].DESCRIPTION;
                        if (ls_desc == description)
                        {
                            rc = rc + 1;
                        }
                    }
                ...
                {
                    dsPayto.DATA[li_row].DESCRIPTION = "";
                    this.SetOnLoadedScript("alert('บัญชีซ้ำ กรุณาตรวจสอบ !!!')");
                }
            }
        }
        ...
        private void of_initpayto()
        {
            ...
            string ls_expcode, ls_expbank, ls_expbranch, ls_expaccid, ls_system, ls_deptno, ls_refslipno;
            Decimal ldc_payout = 0, ldc_asscut = 0;
            ...
            ls_sql = @" select moneytype_code, 
                                   ...
                                   send_system, deptaccount_no , assistreq_docno
                            from   asscontmaster
            ...
            if (dt.Next())
            {
                ...
                ls_refslipno = dt.GetString("assistreq_docno");
            }
            else
            {
                return;
            ...
            //ตรวจสอบรายการหักจากใบคำขอ

            ls_sql = @" select assistcut_amt
                            from   assreqmaster
                            where assist_docno = '" + ls_refslipno + "'";
            dt = WebUtil.QuerySdt(ls_sql);
            if (dt.Next())
            {
                ldc_asscut = Convert.ToDecimal( dt.GetString("assistcut_amt"));
            }
            
            ldc_payout = ldc_payout - ldc_asscut;
            ...
            if (ldc_asscut > 0 )
            {
                dsLoan.InsertAtRow(0);
                dsLoan.DATA[0].METHPAYTYPE_CODE = "LON"; //ต้องใส่ค่าให้มันก่อนไม่งั้นถ้าคนเพิ่มแถวของอีกตารางมันจะหาย
                dsLoan.DATA[0].DESCRIPTION = "หักชำระหนี้";
                dsLoan.DATA[0].ITEMPAY_AMT = ldc_asscut;
                this.of_summary();
            }
        ...
        private void of_initpaycondition()
        {
            ...
            if (dsMain.DATA[0].dead_status == 1)
            {
                ldc_payout = ldc_wtdamt;
            }
        ...
        private void SaveData()
        {
            ...
            for (Int32 i = 0; i < dsPayto.RowCount; i++)
            {
                ...
                if (dsPayto.DATA[i].MONEYTYPE_CODE == "TRN")
                {
                    dsPayto.DATA[i].METHPAYTYPE_CODE = dsMain.DATA[i].send_system;
                }
                else
                {
        ...
        private Int32 of_postdepttran()
        {
            ...
            ls_sendsystem = dsMain.DATA[0].send_system;
            ls_contno = dsMain.DATA[0].ASSCONTRACT_NO;
            ls_desc = dsMain.DATA[0].assisttype_desc;
            ...
            for (Int32 i = 0; i < dsPayto.RowCount; i++)
            {
                ...
                ls_deptno = dsPayto.DATA[0].DEPTACCOUNT_NO;
                ...
                else
                {
                    ...
                    try
                    {
                        ls_sql = WebUtil.SQLFormat(ls_sql, state.SsCoopControl, ls_deptno, ls_memno, ldtm_slipdate, li_seqno, ldc_payamt, ls_slipno,"001");
                        WebUtil.ExeSQL(ls_sql);
                    }
                    catch (Exception ex)
                    {
        ...
        private void of_summary()
        {
            ...
            for (li_row = 0; li_row < dsLoan.RowCount; li_row++)
            {
                ldc_payitem = dsLoan.DATA[li_row].ITEMPAY_AMT;

                ldc_payclr += ldc_payitem;
            }
