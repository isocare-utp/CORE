...
{
    public partial class w_acc_ucf_cashflow_report : PageWebSheet, WebSheet
    {
        ...
        public void CheckJsPostBack(string eventArg)
        {
            if (eventArg == PostInsertRow)
            {
                ...
                wd_list.SetItem(ls_getrow, wd_list.DATA.SEQ_NOColumn,  Convert.ToString(Convert.ToInt32(wd_list.DATA[ls_getrow - 1].SEQ_NO) + 1));
                wd_list.DATA[ls_getrow].ACCOUNT_ACTIVITY = "od";
                wd_list.DATA[ls_getrow].COOP_ID = state.SsCoopControl;
            ...
            else if (eventArg == PostDeleteRow)
            {
                ...
                string sqldelete = "delete from accucfcashflowreport where seq_no = '" + Convert.ToString(Convert.ToInt32(wd_list.DATA[ls_getrow].SEQ_NO)) + "'";
                WebUtil.Query(sqldelete);
                exx.SQL.Add(sqldelete);
                exx.Execute();
                LtServerMessage.Text = WebUtil.CompleteMessage("delete " + seq_no);
