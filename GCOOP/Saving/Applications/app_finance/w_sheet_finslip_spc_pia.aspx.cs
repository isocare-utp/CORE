...
{
    public partial class w_sheet_finslip_spc_pia : PageWebSheet, WebSheet
    {
        n_financeClient fin;
        ...
        decimal taxamt, itemamt, vatamout; 
        ...
        public void WebSheetLoadBegin()
        {
            try
            {
                ...
                //DwMain.SetItemDecimal(1, "retail_flag", 3);
            }
            catch (Exception ex)
            {
            ...
            for (row = 1; row <= rownum; row++)
            {
                ...
                if (tax_flag != 0 && itemPayAmt > 0)
                {
                    ...
                    try
                    {
                        try
                        {
                            result = fin.of_itemcaltax(state.SsWsPass, state.SsCoopId, recvpay, vatflag, taxCode, itemPayAmt,ref taxamt,ref itemamt,ref vatamout);
                        }
                        catch
                        {
                        ...
                        if (taxFlag == 1)
                        {
                            ...
                            DwItem.SetItemDecimal(row, "vat_amt", vatamout);
                        }
                    }
        ...
        protected void PostCalTax()
        {
            try
            {
                ...
                {
                    mainXml = DwMain.Describe("Datawindow.Data.Xml");
                    result = fin.of_caltax(state.SsWsPass, ref mainXml, ref taxdetXml).ToString();
                    DwMain.Reset();
        ...
        {
            Int16 pay_recv_status;
            try
            {
                pay_recv_status = Convert.ToInt16(DwMain.GetItemDecimal(1, "pay_recv_status"));
            }
            catch { pay_recv_status = 9; }
        ...
        protected void PostInitMember()
        {
            try
            {
                ...
                if (DwTax.RowCount == 1)
                {
                    ...
                    result = fin.of_getaddress(state.SsWsPass,ref taxaddr,ref taxid, state.SsCoopId, memberNo, memberFlag).ToString();
