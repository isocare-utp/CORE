...
{
    public partial class w_sheet_finslip_spc_piapost : PageWebSheet, WebSheet
    {
        n_financeClient fin;
        ...
        public void WebSheetLoadBegin()
        {
            try
            {
                ...
                if (!IsPostBack)
                {
                    ...
                    DwUtil.RetrieveDataWindow(dw_data, "finslip_spc.pbl", null, state.SsCoopId);
                }
                else
                {
                    ...
                    this.RestoreContextDw(dw_data,null);
                }
        ...
        public void SaveWebSheet()
        {
            ...
            //            result = fin.OfPostPayRecv_pea(state.SsWsPass, mainXml, itemXml, taxXml, state.SsApplication);
            //        }
            //        catch (Exception err)
            //        {
            ...
            //            result = fin.OfPostPayRecv(state.SsWsPass, mainXml, itemXml, taxXml, state.SsApplication);
            //        }
            //        catch (Exception err)
            //        {
            //            LtServerMessage.Text = WebUtil.WarningMessage(err.Message);
            ...
            try
            {
                ...
                // slip_no 
                // member_no 
                //update payment_status,entry_id finslip
                decimal payment_status = 0;
                string slip_no="";
                string member_no = "";
                string sqlUpdate = "";
                string sqlUpdateChqstm = "";
                int count = dw_data.RowCount;
                ...
                for (int i = 1; i <= count; i++)
                {
                    ...
                    sqlUpdate = "";
                    slip_no = "";
                    payment_status = dw_data.GetItemDecimal(i, "payment_status");
                    if (payment_status == 1) {
                        slip_no = dw_data.GetItemString(i, "slip_no");
                        sqlUpdate = "update finslip set payment_status=1 ,entry_id={0} where slip_no={1} and coop_id={2}";
                        sqlUpdate = WebUtil.SQLFormat(sqlUpdate, state.SsUsername, slip_no, state.SsCoopId);

                        sqlUpdateChqstm = "update finchqeuestatement set chqeue_status = 1,receive_status = 1 ,receive_date={0},receive_entry_id={1},receive_machine_id={2} where coop_id={3} and refer_slipno={4}";
                        sqlUpdateChqstm = WebUtil.SQLFormat(sqlUpdateChqstm, state.SsWorkDate, state.SsUsername, state.SsClientIp, state.SsCoopId, slip_no);

                        exe.SQL.Add(sqlUpdate);
                        exe.SQL.Add(sqlUpdateChqstm);
                    }
                }
        ...
        private void InitBegin(Int16 ai_recvpay, ref String recOrPay)
        {
            try
            {
                ...
                resultXml = fin.of_init_payrecv_slip_1(state.SsWsPass, state.SsCoopId, state.SsUsername, state.SsClientIp, state.SsWorkDate, ai_recvpay, ref slipmainXml);

                DwTax.Reset();
                DwMain.Reset();
                DwItem.Reset();

                DwMain.ImportString(slipmainXml, FileSaveAsType.Xml);
                tDwMain.Eng2ThaiAllRow();
        ...
        protected void PostCalTax()
        {
            try
            {
                ...
                {
                    mainXml = DwMain.Describe("Datawindow.Data.Xml");
                    result = fin.of_caltax(state.SsWsPass, ref mainXml, ref taxdetXml);
                    DwMain.Reset();
        ...
        protected void PostInitMember()
        {
            try
            {
                Int32 main;
                Int16 memberFlag;
                String mainXml = "",taxaddr = "",taxid ="", memberNo;
                Int32 result;
