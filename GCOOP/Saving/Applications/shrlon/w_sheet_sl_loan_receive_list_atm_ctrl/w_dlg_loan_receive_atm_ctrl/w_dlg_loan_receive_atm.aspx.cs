...
namespace Saving.Applications.shrlon.dlg.w_dlg_loan_receive_atm_ctrl
{
    public partial class w_dlg_loan_receive_atm : PageWebDialog, WebDialog
    {
        [JsPostBack]
        ...
        public void CheckJsPostBack(string eventArg)
        {
            if (eventArg == PostOperateFlag)
            {
                ...
                decimal bfloanapprove_amt = dsMain.DATA[0].BFLOANAPPROVE_AMT - dsMain.DATA[0].PAYOUT_AMT; //tomy
                if (operate_flag == 1)
                {
                    ...
                    {
                        if (bfloanapprove_amt < bfshrcont_balamt)
                        {
                            principal_payamt = bfloanapprove_amt; //bfshrcont_balamt - interest_period - bfintarr_amt;
                        }
                        else
                        {
            ...
            else if (eventArg == PostRcvperiod)
            {
                ...
                if (rcvperiod_flag == 1)
                {
                    ...
                    //tomy 05/05/2020 รับบางส่วน ส่งปกติ และปรับยอดรับสุทธิให้เลย
                    dsMain.DATA[0].PAYOUT_AMT = dsMain.DATA[0].PAYOUTCLR_AMT;
        ...
        private void of_initloanrcv()
        {
            try
            {
                ...
                //tomy
                dsMain.DATA[0].RCVPERIOD_FLAG = 1;
                dsMain.DATA[0].LNPAYMENT_STATUS = 1;
                dsMain.FindDropDownList(0, dsMain.DATA.LNPAYMENT_STATUSColumn).Enabled = true;
                    //tomy 05/05/2020 รับบางส่วน ส่งปกติ และปรับยอดรับสุทธิให้เลย
                dsMain.DATA[0].PAYOUT_AMT = dsMain.DATA[0].PAYOUTCLR_AMT;
                ...
                if (result == 1)
                {
                    ...
                    if (print == 1)
                    {
                        ...
                        try
                        {
                            ...
                            slipout_no = strPayOut.payoutslip_no;
                            ...
                            else if (state.SsCoopControl == "040001") //STK
                            {

                                this.SetOnLoadedScript(" parent.PrintSlipoutstk(\"" + slipout_no + "\");");
                            }
                        }
                        catch (Exception ex2)
                        {
            ...
            if ((currentLoan) >= loans.Length)
            {
                ...
                else
                {
                    ...
                    if (print == 1)
                    {
                        ...
                        {
                           // string  slipin_no = strPayOut.payinslip_no;
                            this.SetOnLoadedScript(" parent.PrintSlipoutGsb(\"" + slip_no + "\"); parent.RemoveIFrame();");
                        }*/
                        else
                        {
        ...
        public void calItemPay()
        {
            ...
            //tomy 05/05/2020 รับบางส่วน ส่งปกติ และปรับยอดรับสุทธิให้เลย ATM 
            dsMain.DATA[0].PAYOUTCLR_AMT = payoutclr_amt + dsMain.DATA[0].RETURNETC_AMT;
            dsMain.DATA[0].PAYOUT_AMT = dsMain.DATA[0].PAYOUTCLR_AMT + dsMain.DATA[0].RETURNETC_AMT;
            dsMain.DATA[0].PAYOUTNET_AMT = dsMain.DATA[0].PAYOUT_AMT - dsMain.DATA[0].PAYOUTCLR_AMT;
            dsMain.DATA[0].RCVPERIOD_FLAG = 1;
            dsMain.DATA[0].LNPAYMENT_STATUS = 1;
            dsMain.FindDropDownList(0, dsMain.DATA.LNPAYMENT_STATUSColumn).Enabled = true;
