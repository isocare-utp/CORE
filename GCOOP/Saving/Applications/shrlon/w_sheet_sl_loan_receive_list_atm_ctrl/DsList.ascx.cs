...
namespace Saving.Applications.shrlon.w_sheet_sl_loan_receive_list_atm_ctrl
{
  ...
  SELECT 'CON' as lnrcvfrom_code,   
   ...
   WHERE ( LNREQLOAN.MEMBER_NO = MBMEMBMASTER.MEMBER_NO ) and  
         ...
         ( ( lnreqloan.loanrequest_status = 13 ) ) AND
		 LNLOANTYPE.ATM_FLAG =  1 AND  
		  ( LNREQLOAN.LOANREQUEST_DOCNO  in  ( SELECT LNREQLOANCLR.LOANREQUEST_DOCNO FROM LNREQLOANCLR WHERE LNREQLOANCLR.CLEAR_STATUS > 0 AND LNREQLOAN.LOANREQUEST_DOCNO = LNREQLOANCLR.LOANREQUEST_DOCNO ) or
		   LNREQLOAN.LOANREQUEST_DOCNO  in  ( SELECT LNREQLOANCLROTHER.LOANREQUEST_DOCNO FROM LNREQLOANCLROTHER WHERE LNREQLOANCLROTHER.CLEAR_STATUS > 0 AND LNREQLOAN.LOANREQUEST_DOCNO = LNREQLOANCLROTHER.LOANREQUEST_DOCNO ) ) AND
          ( LNREQLOAN.LOANREQUEST_DOCNO not in  ( SELECT SLSLIPPAYOUT.LOANREQUEST_DOCNO FROM SLSLIPPAYOUT WHERE SLSLIPPAYOUT.SLIP_STATUS > 0 and SLSLIPPAYOUT.SLIPTYPE_CODE = 'LWD'
		 AND LNLOANTYPE.LOANTYPE_CODE = SLSLIPPAYOUT.SHRLONTYPE_CODE AND SLSLIPPAYOUT.LOANREQUEST_DOCNO is not null)) AND
		 ( LNREQLOAN.LOANREQUEST_DOCNO  in  ( SELECT LNCONTMASTER.LOANREQUEST_DOCNO FROM LNCONTMASTER WHERE LNCONTMASTER.CONTRACT_STATUS > 0 AND LNREQLOAN.LOANREQUEST_DOCNO = LNCONTMASTER.LOANREQUEST_DOCNO  ))    
 ";
 ...
 ";
        ...
        //ใช้งานอยู่ ปุ่ม ค้นหา
        public void RetrieveListEntryGroup(string entry)
        {
            String sql = @"
            SELECT 'CON' as lnrcvfrom_code,   
	        LNREQLOAN.COOP_ID,   
	        ...
	        LNREQLOAN.MEMBER_NO,   
	        LNREQLOAN.LOANAPPROVE_AMT withdrawable_amt,   
	        LNLOANTYPE.PREFIX,   
	        MBUCFPRENAME.PRENAME_DESC,   
	        MBMEMBMASTER.MEMB_NAME,   
	        MBMEMBMASTER.MEMB_SURNAME,   
            ...
            FROM LNREQLOAN
            inner join MBMEMBMASTER on LNREQLOAN.MEMBER_NO = MBMEMBMASTER.MEMBER_NO and LNREQLOAN.MEMCOOP_ID = MBMEMBMASTER.COOP_ID
            inner join MBUCFPRENAME on MBMEMBMASTER.PRENAME_CODE = MBUCFPRENAME.PRENAME_CODE
            inner join LNLOANTYPE  on LNREQLOAN.LOANTYPE_CODE = LNLOANTYPE.LOANTYPE_CODE and LNREQLOAN.COOP_ID = LNLOANTYPE.COOP_ID
            WHERE LNREQLOAN.MEMCOOP_ID = {0}  AND 
            LNREQLOAN.ENTRY_ID like {1}   AND
            ( ( lnreqloan.loanrequest_status = 13 ) ) AND
		    LNLOANTYPE.ATM_FLAG =  1 AND  
		     ( LNREQLOAN.LOANREQUEST_DOCNO  in  ( SELECT LNREQLOANCLR.LOANREQUEST_DOCNO FROM LNREQLOANCLR WHERE LNREQLOANCLR.CLEAR_STATUS > 0 AND LNREQLOAN.LOANREQUEST_DOCNO = LNREQLOANCLR.LOANREQUEST_DOCNO ) or
		   LNREQLOAN.LOANREQUEST_DOCNO  in  ( SELECT LNREQLOANCLROTHER.LOANREQUEST_DOCNO FROM LNREQLOANCLROTHER WHERE LNREQLOANCLROTHER.CLEAR_STATUS > 0 AND LNREQLOAN.LOANREQUEST_DOCNO = LNREQLOANCLROTHER.LOANREQUEST_DOCNO ) ) AND
   ...
   ";
            ...
            sql = WebUtil.SQLFormat(sql, state.SsCoopId, entry);
            DataTable dt = WebUtil.Query(sql);
            this.ImportData(dt);
        }
...
";
   ...
   WHERE ( MBMEMBMASTER.MEMBER_NO = LNCONTMASTER.MEMBER_NO ) and  
         ...
         ( LNCONTMASTER.LOANCONTRACT_NO not in ( SELECT SLSLIPPAYOUT.LOANCONTRACT_NO FROM SLSLIPPAYOUT WHERE SLSLIPPAYOUT.SLIP_STATUS > 0 )) )   
";
...
lnreqloan.printpay_date as slip_date
   ...
   WHERE ( LNREQLOAN.MEMBER_NO = MBMEMBMASTER.MEMBER_NO ) and  
         ...
         order by LNREQLOAN.LOANCONTRACT_NO";
  ...
  SELECT 'CON' as lnrcvfrom_code,   
   ...
   WHERE ( LNREQLOAN.MEMBER_NO = MBMEMBMASTER.MEMBER_NO ) and  
         ...
         ( ( lnreqloan.loanrequest_status = 13 ) ) AND
		 LNLOANTYPE.ATM_FLAG =  1 AND  
		 ( LNREQLOAN.LOANREQUEST_DOCNO  in  ( SELECT LNREQLOANCLR.LOANREQUEST_DOCNO FROM LNREQLOANCLR WHERE LNREQLOANCLR.CLEAR_STATUS > 0 AND LNREQLOAN.LOANREQUEST_DOCNO = LNREQLOANCLR.LOANREQUEST_DOCNO )) AND
         ( LNREQLOAN.LOANREQUEST_DOCNO not in  ( SELECT SLSLIPPAYOUT.LOANREQUEST_DOCNO FROM SLSLIPPAYOUT WHERE SLSLIPPAYOUT.SLIP_STATUS > 0 and SLSLIPPAYOUT.SLIPTYPE_CODE = 'LWD'
		 AND LNLOANTYPE.LOANTYPE_CODE = SLSLIPPAYOUT.SHRLONTYPE_CODE AND SLSLIPPAYOUT.LOANREQUEST_DOCNO is not null)) AND
		 ( LNREQLOAN.LOANREQUEST_DOCNO  in  ( SELECT LNCONTMASTER.LOANREQUEST_DOCNO FROM LNCONTMASTER WHERE LNCONTMASTER.CONTRACT_STATUS > 0 AND LNREQLOAN.LOANREQUEST_DOCNO = LNCONTMASTER.LOANREQUEST_DOCNO  ))    
     
         order by lnreqloan.entry_date,lnreqloan.loanrequest_docno";
