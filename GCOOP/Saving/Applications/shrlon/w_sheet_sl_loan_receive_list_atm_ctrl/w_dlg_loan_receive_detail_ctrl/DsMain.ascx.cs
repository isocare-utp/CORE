...
namespace Saving.Applications.shrlon.w_sheet_sl_loan_receive_list_atm_ctrl.w_dlg_loan_receive_detail_ctrl
{
    public partial class DsMain : DataSourceFormView
    {
        ...
        public void RetrieveMain(String loancontract_no)
        {
		    ...
		    when LNCONTMASTER.LOANPAYMENT_TYPE = 2 then 'คงยอด' end )
							         as PAYMENT_TYPE, 
		   (case when LNLOANTYPE.CONTRACTINT_TYPE = 0 then 'ตามประกาศ' 
		   when LNLOANTYPE.CONTRACTINT_TYPE = 1 then 'คงที่มีระยะเวลา'
           when LNLOANTYPE.CONTRACTINT_TYPE = 2 then 'คงที่ตลอด อัตรา' end ) 
							        as CONTRACTINT_TYPE,
            ...
            when LNCONTMASTER.PAYMENT_STATUS = -11 then 'งดส่งต้นเงิน' end ) 
                                    as PAY_STATUS ,
                               ...
                               FROM LNLOANTYPE,
                                    MBUCFPRENAME,
             MBMEMBMASTER LEFT JOIN MBUCFMEMBGROUP on MBMEMBMASTER.MEMBGROUP_CODE = MBUCFMEMBGROUP.MEMBGROUP_CODE                                 
			              LEFT JOIN CMUCFBANK on MBMEMBMASTER.EXPENSE_BANK= CMUCFBANK.BANK_CODE
			              LEFT JOIN CMUCFBANKBRANCH on MBMEMBMASTER.EXPENSE_BRANCH = CMUCFBANKBRANCH.BRANCH_ID,
             LNCONTMASTER LEFT JOIN LNUCFLOANOBJECTIVE on LNCONTMASTER.LOANOBJECTIVE_CODE = LNUCFLOANOBJECTIVE.LOANOBJECTIVE_CODE
