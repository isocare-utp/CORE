...
namespace Saving.Applications.shrlon.dlg
{
    public partial class w_dlg_sl_loanrequest_cleardet : PageWebDialog, WebDialog
    {
        ...
        public void WebDialogLoadBegin()
        {
            ...
            else
            {
                try
                {
                    ...
                    {
                        int result = shrlonService.of_viewloanclrdetail(state.SsWsPass, state.SsCoopId, contractNo, xmlClear, ref xmlDetail);

                        if (result == 1)
                        {
                            dw_cleardet.Reset();
                           // dw_cleardet.ImportString(xmlDetail, FileSaveAsType.Xml);
                            DwUtil.ImportData(xmlDetail, dw_cleardet, null, FileSaveAsType.Xml);
                        }
                        else
                        {
                            dw_cleardet.Reset();
        ...
        {
            if(dw_cleardet.RowCount>1)
            {
            DwUtil.DeleteLastRow(dw_cleardet);
            }
