...
namespace Saving.Applications.shrlon.ws_sl_slip_pay_ctrl
{
    public partial class DsMain : DataSourceFormView
    {
        ...
        public void DdSliptype()
        {
            string sql = @" 
              	     SELECT   
		rtrim(ltrim( SLUCFSLIPTYPE.SLIPTYPE_CODE)) AS SLIPTYPE_CODE,
        ...
        public void DdFromAccId(string sliptype_code, string moneytype_code)
        {
            string sql = @"SELECT 
                        CMUCFTOFROMACCID.MONEYTYPE_CODE, 
                         ...
                         1 as sorter
                 FROM ACCMASTER RIGHT JOIN  CMUCFTOFROMACCID on   ACCMASTER.COOP_ID = CMUCFTOFROMACCID.COOP_ID 
										and CMUCFTOFROMACCID.ACCOUNT_ID =ACCMASTER.ACCOUNT_ID
                 
WHERE (CMUCFTOFROMACCID.SLIPTYPE_CODE = '" + sliptype_code + "') and (CMUCFTOFROMACCID.MONEYTYPE_CODE = '" + moneytype_code + @"')
        ...
        public void RetrieveMain(string payinslip_no)
        {
            String sql = @"SELECT SLSLIPPAYIN.MEMBGROUP_CODE,   
                    ...
                    FROM  
                         ...
                         CMUCFMONEYTYPE,   
                         MBUCFPRENAME  ,
SLSLIPPAYIN LEFT JOIN MBUCFMEMBGROUP on SLSLIPPAYIN.MEMBGROUP_CODE = MBUCFMEMBGROUP.MEMBGROUP_CODE
