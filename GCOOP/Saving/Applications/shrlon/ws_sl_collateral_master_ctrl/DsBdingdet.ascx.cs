...
namespace Saving.Applications.shrlon.ws_sl_collateral_master_ctrl
{
    public partial class DsBdingdet : DataSourceRepeater
    {
        ...
        public void Ddfloor()
        {
            string sql = @"select floor_no,floor_desc, 1 as sorter
                            from lnucfcollmastfloor  
                            union
                            select 0,'',0 from dual order by sorter, floor_no";
            DataTable dt = WebUtil.Query(sql);
            this.DropDownDataBind(dt, "floor_no", "floor_desc", "floor_no");
        }
    }
}