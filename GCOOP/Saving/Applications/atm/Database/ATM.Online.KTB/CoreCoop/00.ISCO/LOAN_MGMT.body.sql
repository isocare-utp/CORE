...
--------------------------------------------------------
  ...
  CREATE OR REPLACE PACKAGE BODY "LOAN_MGMT" AS
	...
	begin
        ...
        Function CalcContrcPerdPay(LoanType varchar2, LoanPayType varchar2, IntRoundType number, PayRoundType number, FixPayCalcType number, PayRoundFactor number, PrnBal in out number, PerdPay in out number, LastPerdPay in out number) return number
	is
	Payment				number(15, 2);
	ModPay				number(15, 2);
	IntRate				number(15, 2);
	...
	where 	l1.loanintrate_code = LoanType and
...
/*		DayNo := CalcIntTo - CalcIntFrom;
		...
		FOR i IN 1..intrate.COUNT LOOP
			if TempAmt >= MinAmt(i) and TempAmt <= intrate(i).lower_amt then
...
--		if (PrnBal = 0) or trunc(CalcIntTo) < trunc(CalcIntFrom) then return 0.00; end if;
		...
		if GBfSts = 1 then
            TmpPrnBal_:=BfPrnBal+PrnBal;
        		PerdPayAmt := CalcContrcPerdPay(LoanType, LoanPayType, IntRoundType, PayRoundType, FixPayCalcType, PayRoundFactor, TmpPrnBal_, PerdPayAll, LastPerdPay);
