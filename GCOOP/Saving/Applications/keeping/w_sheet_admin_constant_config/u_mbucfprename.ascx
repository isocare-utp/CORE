<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="u_mbucfprename.ascx.cs" Inherits="Saving.Applications.keeping.u_mbucfprename" %>
<%@ Register Assembly="WebDataWindow" Namespace="Sybase.DataWindow.Web" TagPrefix="dw" %>

<script type="text/javascript">
        function OnButtonClick(sender, row, name){
            if(name == "b_delete"){
                var detail = "รหัส " + objdw_main.GetItem(row, "prename_code");
                detail += " : " + objdw_main.GetItem(row, "prename_desc");
...
<dw:WebDataWindowControl ID="dw_main" runat="server"
    DataWindowObject="d_mbucfprename" 
