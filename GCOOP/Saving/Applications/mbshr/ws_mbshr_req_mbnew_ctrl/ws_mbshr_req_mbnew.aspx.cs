...
{
    public partial class ws_mbshr_req_mbnew : PageWebSheet, WebSheet
    {
        [JsPostBack]
        ...
        public void CheckJsPostBack(string eventArg)
        {
            ...
            {
                if (int.Parse(dsMain.DATA[0].MEMBDATEFIX_FLAG) == 1)
                {
            ...
            else if (eventArg == PostSalary)
            {
                ...
                Decimal member_type = decimal.Parse(dsMain.DATA[0].MEMBER_TYPE);
                Decimal salary_amount, incomeetc_amt;
                try { salary_amount = decimal.Parse(dsMain.DATA[0].SALARY_AMOUNT); }
                catch { salary_amount = 0; }
                try { incomeetc_amt = decimal.Parse(dsMain.DATA[0].INCOMEETC_AMT); }
        ...
        public void NewClear()
        {
            ...
            dsMain.DATA[0].APPLY_DATE = String.Format("{0:dd/MM/yyyy}", state.SsWorkDate); 
            ...
            //    {
            //        dsMoneytr.DATA[i].MONEYTYPE_CODE = "SAL";
            //    }
            //}
        }
        ...
        {
            Decimal share_mth = 0;
            member_type = decimal.Parse(dsMain.DATA[0].MEMBER_TYPE);
        ...
        public void age()
        {
            try
            {
                DateTime birth_date = DateTime.Parse(dsMain.DATA[0].BIRTH_DATE);
