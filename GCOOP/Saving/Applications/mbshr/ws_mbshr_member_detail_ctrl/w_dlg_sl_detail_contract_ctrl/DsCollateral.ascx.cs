...
namespace Saving.Applications.mbshr.ws_mbshr_member_detail_ctrl.w_dlg_sl_detail_contract_ctrl
{
    public partial class DsCollateral : DataSourceRepeater
    {
		...
		round((case when lt.collreturnval_status = 1 then (

 
(Case when lm.principal_balance is null then 0 else lm.principal_balance end) +
(Case when lm.withdrawable_amt is null then 0 else lm.withdrawable_amt end  )

 ) * ( lc.collactive_percent / lc.collbase_percent ) else
