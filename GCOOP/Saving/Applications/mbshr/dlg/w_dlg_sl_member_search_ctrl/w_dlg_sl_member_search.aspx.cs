...
(MBUCFPRENAME.PRENAME_DESC +''+ MBMEMBMASTER.MEMB_NAME+' '+MBMEMBMASTER.MEMB_SURNAME) AS FULLNAME, 
            ...
            for (int i = 0; i < dsList.RowCount; i++)
            {
                if (dsList.DATA[i].RESIGN_STATUS == 1)
                {
                    ...
                    dsList.FindTextBox(i, "MEMBGROUP_DESC").BackColor = Color.Red;
                    dsList.FindTextBox(i, "SALARY_ID").BackColor = Color.Red;
                }
                else
                {
                    dsList.FindTextBox(i, "MEMBER_NO").BackColor = Color.White;
                    dsList.FindTextBox(i, "FULLNAME").BackColor = Color.White;
                    dsList.FindTextBox(i, "MEMBGROUP_DESC").BackColor = Color.White;
                    dsList.FindTextBox(i, "SALARY_ID").BackColor = Color.White;
                }
            }
        }
    }
