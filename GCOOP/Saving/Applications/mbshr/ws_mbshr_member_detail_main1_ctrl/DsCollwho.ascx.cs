...
namespace Saving.Applications.mbshr.ws_mbshr_member_detail_main_ctrl
{
    public partial class DsCollwho : DataSourceRepeater
    {
		...
		round((case when lt.collreturnval_status = 1 then (case when lm.principal_balance is null then 0 else  lm.principal_balance end  )+(case when  lm.withdrawable_amt is null then 0 else   lm.withdrawable_amt end  ) * ( lc.collactive_percent / lc.collbase_percent ) else
