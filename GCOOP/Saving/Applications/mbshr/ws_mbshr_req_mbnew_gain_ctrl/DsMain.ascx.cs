...
namespace Saving.Applications.mbshr.ws_mbshr_req_mbnew_gain_ctrl
{
    public partial class DsMain : DataSourceFormView
    {
        ...
        public void DdAppltypeCode()
        {
		 ...
		 'ค่าธรรมเนียมแรกเข้า'+appltype_desc +'  '+ str(first_fee,8,2)+'บาท'  as appltype_desc 
            from mbucfappltype
            where coop_id = {0}
        ...
        public void DdConcern()
        {
            string sql = @" select concern_code, gain_concern from mbucfgainconcern";
            DataTable dt = WebUtil.Query(sql);
            this.DropDownDataBind(dt, "mather_name", "gain_concern", "concern_code");
        }
        public void DdMembgroupCode()
        {
            string sql = @"
             select membgroup_code,
              rtrim(ltrim(membgroup_code))+' - '+membgroup_desc as  membgroup_desc
                , 1 as sorter
            from mbucfmembgroup
            where coop_id = {0}
            union
        ...
        {
            string sql = @"select position_code, position_desc, 1 as sorter
                            from mbucfposition
                         union select '','', 0  order by sorter,  position_code ASC";
            DataTable dt = WebUtil.Query(sql);
            this.DropDownDataBind(dt, "position_code", "position_desc", "position_code");
        }

        public void DdLevelCode()
        {
            string sql = @"select rtrim(ltrim(level_code)) as level_code, level_desc, 1 as sorter
                            from mbucflevel
                         union select '','', 0  order by sorter,  level_code ASC";
            DataTable dt = WebUtil.Query(sql);
            this.DropDownDataBind(dt, "level_code", "level_desc", "level_code");
        }
    }
}