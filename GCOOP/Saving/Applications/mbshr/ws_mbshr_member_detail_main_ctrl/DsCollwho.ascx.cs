...
namespace Saving.Applications.mbshr.ws_mbshr_member_detail_main_ctrl
{
    public partial class DsCollwho : DataSourceRepeater
    {
		...
		lc.loancontract_no,
		ISNULL( lm.principal_balance, 0 )+ISNULL( lm.withdrawable_amt, 0 ) as prnbal_amt,
		lc.collactive_percent,mb.resign_status,
		round((case when lt.collreturnval_status = 1 then ( ISNULL( lm.principal_balance, 0 )+ISNULL( lm.withdrawable_amt, 0 ) ) * ( lc.collactive_percent / lc.collbase_percent ) else
