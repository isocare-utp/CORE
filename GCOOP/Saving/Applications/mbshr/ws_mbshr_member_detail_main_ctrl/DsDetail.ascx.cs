        ...
        public void RetrieveDetail(String ls_member_no)
        {
            String sql = @"  
                     SELECT MBMEMBMASTER.MEMBER_NO ,          
                           ...
                           MBMEMBMASTER.REMARK ,        
                           MBMEMBMASTER.POSITION_DESC as POSITION_DESC ,
...
(case when MBMEMBMASTER.addr_mobilephone = '' then MBMEMBMASTER.addr_phone
         when MBMEMBMASTER.addr_phone = '' then MBMEMBMASTER.curraddr_phone 
						...
						+ '.'+
						CONVERT(VARCHAR, round( ( DATEDIFF(month, birth_date,getdate()) % 12 ), 0 )) as age,
						...
						CONVERT(VARCHAR, round( ( DATEDIFF(month, work_date,getdate()) % 12 ), 0 )) as work_age,  
                           substring(dbo.ftcm_calagemth(birth_date,retry_date),1,2) as retry_age, 
						CONVERT(VARCHAR, (round( DATEDIFF(month, member_date,getdate()) - ( DATEDIFF(month, member_date,getdate()) % 12 ) , 0 )/12) ) 
						+ '.'+
						CONVERT(VARCHAR, round( ( DATEDIFF(month, member_date,getdate()) % 12 ), 0 )) as memb_age,
