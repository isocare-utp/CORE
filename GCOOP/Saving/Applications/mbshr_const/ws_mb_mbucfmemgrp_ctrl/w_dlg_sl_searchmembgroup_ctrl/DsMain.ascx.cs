...
namespace Saving.Applications.mbshr_const.ws_mb_mbucfmemgrp_ctrl.w_dlg_sl_searchmembgroup_ctrl
{
    public partial class DsMain : DataSourceFormView
    {
        ...
        {
            string sql = @"
            select membgroup_control,   
            membgroup_control +' '+ membgroup_controldesc as membgroup_desc , 1 as sorter
            from mbucfmembgroupcontrol
            union 
            select '','',0 from dual order by sorter,membgroup_control asc";
            sql = WebUtil.SQLFormat(sql, state.SsCoopControl);
            DataTable dt = WebUtil.Query(sql);
            this.DropDownDataBind(dt, "membgroup_control", "membgroup_desc", "membgroup_control");
        }
